...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if downsample is not None:
            self.downsample_sn = spiking_neuron(hids=planes * self.expansion, **deepcopy(kwargs))
        self.stride = stride
        self.cnf = cnf
...
class SEWResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, cnf: str = None, pe: bool = False, spiking_neuron: callable = None,
                 T=4, **kwargs):
        super().__init__()
        self.T = T
        if norm_layer is None:
    ...
    def _forward_impl(self, x):
        if self.pe is None:
            ...
            x = x.unsqueeze(0).repeat(self.T, 1, 1, 1, 1) + self.pe
            x = self.conv1(x)
            x = self.bn1(x)
...
def sew_resnet18(pretrained=False, progress=True, cnf: str = None, spiking_neuron: callable = None, **kwargs):
    ...
    :type kwargs: dict
    :param channels: (new) dimension of the input channels for the spiking neuron
    :type channels: int
