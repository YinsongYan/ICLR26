...
from neuron import enh_IFNode
import enh_sew_resnet, utils
# from sew_resnet import model_urls
...
#     """
#     Apply Xavier initialization to all layers of the given model.
#     """
#     for layer in model.modules():
#         if isinstance(layer, (nn.Linear, nn.Conv2d)):  # Apply to Linear or Conv2d layers
#             nn.init.xavier_uniform_(layer.weight)  # Xavier Uniform initialization
#             if layer.bias is not None:  # Check if bias exists and initialize it
#                 nn.init.zeros_(layer.bias)
#         elif isinstance(layer, nn.BatchNorm2d):  # BatchNorm layers
#             nn.init.ones_(layer.weight)  # Initialize gamma to 1
#             nn.init.zeros_(layer.bias)  # Initialize beta to 0
...
def main(args):
    ...
    print(args)
    output_dir = os.path.join(args.output_dir, 'enh_T1_D4_Rtmax3sigInit_newGNRectsf{1e-2}_ps{0.5}', f'{args.model}_b{args.batch_size}_lr{args.lr}_T{args.T}')
    ...
    if args.model in enh_sew_resnet.__dict__:
        # model = sew_resnet.__dict__[args.model](
        #     pretrained=args.pretrained, cnf='ADD', spiking_neuron=IFNode,
        #     surrogate_function=surrogate.ATan(), T=args.T, step_mode='m', backend='cupy')
        model = enh_sew_resnet.__dict__[args.model](
            pretrained=args.pretrained, cnf='ADD', spiking_neuron=enh_IFNode,
            surrogate_function=surrogate.ATan(), T=args.T, step_mode='m', backend='torch')
        ...
        # for name, param in model.named_parameters():
        #     # Check if the parameter belongs to enh_IFNode and is learnable
        #     if "enh_IFNode" in name and param.requires_grad:
        #         print(name, param.requires_grad)

        # print(model)
        # for name, module in model.named_modules():
        #     if isinstance(module, enh_IFNode):  # Check if the module is an instance of enh_IFNode
        #         print(f"Parameters in {name} (enh_IFNode):")
        #         # Iterate through all named parameters in the module
        #         for param_name, param in module.named_parameters():
        #             print(f"  {name}.{param_name}: {param.size()} (requires_grad={param.requires_grad})")
    ...
    if args.load is not None:
        # model.load_state_dict_from_url(model_urls[arch], strict=False)
...
def parse_args():
    ...
    parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                        help='start epoch')
    parser.add_argument('-lambda_aux', type=float, default=0.001,
