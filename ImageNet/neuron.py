...
class CustomRoundPass(torch.autograd.Function):
    ...
    def forward(ctx, x, scaling_factor):
        y = x.round()
        dis = x - (x.ceil() + 0.5)
        # ctx.save_for_backward(x, x_c)  # Save quantization error (cur_quan_dis)
    ...
    def backward(ctx, grad_output):
        ...
        grad_out_norm = grad_out / grad_out.norm(p=2)  # , dim=0, keepdim=True
        grad_out = grad_out + ctx.scaling_factor * (0.5 - torch.sign(dis) * dis) * grad_out_norm
...
class RealSpiking_M(torch.nn.Module):
    ...
    def forward(self, x, alpha, p):
        ...
        out = round_pass(x) * alpha_reshape
...
class enh_IFNode(BaseNode):
    def __init__(self, hids: int = 64, v_threshold: float = 1., v_reset: Optional[float] = 0.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False, step_mode='s', T=1,
                 backend='torch', store_v_seq: bool = False):
        """
        :param v_threshold: threshold of this neurons layer
        :type v_threshold: float
