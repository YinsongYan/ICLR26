...
from models import spiking_resnet, vgg_model, spiking_vgg
from modules import neuron
from modules.neuron import LIFNode, ParametricLIFNode
from modules.neuron import enh_LIFNode
...
from utils.augmentation import ToPILImage, Resize, ToTensor, RandomFlipAndShift

from utils.eval import TET_loss
# from utils.cifar10_dvs_ori import DVSCifar10
...
def main():
    ...
    parser.add_argument('-tau', default=0.5, type=float, help='a hyperparameter for the LIF model')
    ...
    parser.add_argument('-lambda_aux', type=float, default=0.0,
                        help='the scaling factor for the Aux term in the loss')
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        transform_train = transforms.Compose([
            ...
            # RandomFlipAndShift(max_shift=5, flip_prob=0.5),
        ...
        # # trainset = CIFAR10DVS(root=args.data_dir, data_type='frame', split_by='number', frames_number=args.T, transform=transform_train)
        # # testset = CIFAR10DVS(root=args.data_dir, data_type='frame', split_by='number', frames_number=args.T, transform=transform_test)
        #
    ...
    if args.neuron_model == 'LIF':
        ...
        neuron_model = ParametricLIFNode
    elif args.neuron_model == 'relu':
        neuron_model = ReLU
        args.T = 1
    elif args.neuron_model == 'EnhLIF':
        neuron_model = enh_LIFNode
    else:
        raise NotImplementedError
    ...
    if args.model in ['spiking_resnet18', 'spiking_resnet34', 'spiking_resnet50', 'spiking_resnet101', 'spiking_resnet152']:
        net = spiking_resnet.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                                  ...
                                                  surrogate_function=surrogate_function, step_mode='s', T=args.T, backend='torch' #,backend='cupy'
                                                  )
        print('using Resnet model.')
    elif args.model in ['spiking_vgg11_bn', 'spiking_vgg13_bn', 'spiking_vgg16_bn', 'spiking_vgg19_bn', 'spiking_vgg11_bn_wdrop']:
        net = spiking_vgg.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                                  neuron_dropout=args.drop_rate,
                                                  surrogate_function=surrogate_function, step_mode='s', T=args.T, backend='torch'
                                                  )
        print('using Spiking VGG model.')
    elif args.model in ['vggsnn', 'snn5_noAP']:  # DVScifar10   snn5_noAP use for statistical experiment
        net = vgg_model.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                             neuron_dropout=args.drop_rate,
                                             surrogate_function=surrogate_function, step_mode='s', T=args.T, tau=args.tau, v_reset_mode='hard', backend='torch'
                                             )
        print('using VGGSNN model.')
    else:
        raise NotImplementedError
    ...
    ##########################################################
    ## for vanilla LIF
    # out_dir = os.path.join(args.out_dir, f'train_{args.dataset}_{args.model}_{args.neuron_model}_{args.name}_T{args.T}__vanilla___tau{args.tau}_e{args.epochs}_bs{args.b}_{args.opt}_lr{args.lr}_wd{args.weight_decay}_SG_{args.surrogate}_drop{args.drop_rate}__labelsmoothing{args.label_smoothing}')

    ## for enh_LIF
    out_dir = os.path.join(args.out_dir, f'train_{args.dataset}_{args.model}_{args.neuron_model}_{args.name}_T{args.T}_D{4}_Rtmax3sigInit_newGNRectsf{1e-2}_ps{0.5}___tau{args.tau}_e{args.epochs}_bs{args.b}_{args.opt}_lr{args.lr}_wd{args.weight_decay}_SG_{args.surrogate}_drop{args.drop_rate}__labelsmoothing{args.label_smoothing}')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for data in train_data_loader:
            ...
            # out_all = torch.cat(out_all, 0)  # shape [B*T, num_classes]

            out_all = torch.stack(out_all, dim=1)  # Shape: [B, T, num_classes]
            mean_out = out_all.mean(dim=1)
            ...
            if args.amp:
                with amp.autocast():
                    if args.TET:
                        loss = TET_loss(out_all, label, criterion, args.loss_means, args.loss_lambda)
                    else:
                        loss = criterion(mean_out, label)
            ...
            prec1, prec5 = accuracy(mean_out.data, label.data, topk=(1, 5))
        ...
        with torch.no_grad():
            for data in test_data_loader:
                ...
                out_all = torch.stack(out_all, dim=1)  # Shape: [B, T, num_classes]
                mean_out = out_all.mean(dim=1)  # Compute mean output across time steps
                ...
                test_acc += (mean_out.argmax(1) == label).float().sum().item()
                functional.reset_net(net)
