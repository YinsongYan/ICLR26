...
from models import spiking_resnet_sj, vgg_model, spiking_vgg
...
def main():
    ...
    parser.add_argument('-cutupmix_auto', action='store_true',
                        help='cutupmix autoaugmentation for cifar and tinyimagenet')
    ...
    parser.add_argument('-lambda_aux', type=float, default=0.000,
    ...
    if args.model in ['spiking_resnet18', 'spiking_resnet19', 'spiking_resnet34', 'spiking_resnet50', 'spiking_resnet101',
                      'spiking_resnet152']:
        net = spiking_resnet_sj.__dict__[args.model](pretrained=False, progress=True,
                                                  spiking_neuron=neuron_model, num_classes=num_classes,
                                                  surrogate_function=surrogate_function,
                                                  step_mode='s', T=args.T,
                                                  backend='torch'
                                                  )

        print('using Resnet model.')
    elif args.model in ['spiking_vgg11_bn', 'spiking_vgg13_bn', 'spiking_vgg16_bn', 'spiking_vgg19_bn',
                        'spiking_vgg11_bn_wdrop']:
        net = spiking_vgg.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                               neuron_dropout=args.drop_rate,
                                               surrogate_function=surrogate_function, step_mode='s', T=args.T,
                                               backend='torch'
                                               )
        print('using Spiking VGG model.')
    elif args.model in ['vggsnn', 'snn5_noAP']:  # DVScifar10
        net = vgg_model.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                             neuron_dropout=args.drop_rate,
                                             surrogate_function=surrogate_function, step_mode='s', T=args.T,
                                             tau=args.tau, v_reset_mode='hard', backend='torch'
                                             )
    ...
    ##########################################################
    # For Enh-ResNet19
    out_dir = os.path.join(args.out_dir, f'train_{args.dataset}_{args.model}_{args.neuron_model}_sj__{args.name}_T{args.T}_D{4}_Rtmax3sigInit_newGNRectsf{1e-2}_ps{0.5}___tau{args.tau}_e{args.epochs}_bs{args.b}_{args.opt}_lr{args.lr}_wd{args.weight_decay}_SG_{args.surrogate}_drop{args.drop_rate}__labelsmoothing{args.label_smoothing}')
