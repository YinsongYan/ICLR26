...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = spiking_neuron(hids=planes, **deepcopy(kwargs))
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(planes)
        self.sn2 = spiking_neuron(hids=planes, **deepcopy(kwargs))
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.sn1 = spiking_neuron(hids=width, **deepcopy(kwargs))
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.sn2 = spiking_neuron(hids=width, **deepcopy(kwargs))
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.sn3 = spiking_neuron(hids=planes * self.expansion, **deepcopy(kwargs))
        self.downsample = downsample
        self.stride = stride
...
#         self.sn1 = spiking_neuron(hids=self.inplanes, **deepcopy(kwargs))
...
class SpikingResNet19(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.bn1 = norm_layer(self.inplanes)
        self.sn1 = spiking_neuron(hids=self.inplanes, **deepcopy(kwargs))
        self.sn_out = spiking_neuron(hids=256, **deepcopy(kwargs))
        # self.maxpool = layer.MaxPool2d(kernel_size=3, stride=2, padding=1)
        ...
        self.classifier = nn.Sequential(
            layer.Linear(512 * block.expansion, 256, bias=True),
            self.sn_out,
            layer.Linear(256, num_classes, bias=True),
        )
...
def _spiking_resnet19(arch, block, layers, pretrained, progress, spiking_neuron, **kwargs):
    model = SpikingResNet19(block, layers, spiking_neuron=spiking_neuron, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
def spiking_resnet19(pretrained=False, progress=True, spiking_neuron: callable=None, **kwargs):
    """
    :param pretrained: If True, the SNN will load parameters from the ANN pre-trained on ImageNet
    :type pretrained: bool
    :param progress: If True, displays a progress bar of the download to stderr
    ...
    :param kwargs: kwargs for `spiking_neuron`
    :type kwargs: dict
    :return: Spiking ResNet-19
    :rtype: torch.nn.Module

    A spiking version of ResNet-19 model from `"Going Deeper With Directly-Trained Larger Spiking Neural Networks" <https://arxiv.org/pdf/2011.05280>`_
    """

    return _spiking_resnet19('resnet19', BasicBlock, [3, 3, 2], pretrained, progress, spiking_neuron, **kwargs)
