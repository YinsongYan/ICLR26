...
class ElasticBiSpiking_MLN(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, alpha, act_clip_val=1, symmetric=True):
        # ctx.save_for_backward(input)
        # ctx.save_for_backward(input, alpha)
        # ctx.other = act_clip_val
        if symmetric==True:
            Qn = -act_clip_val
            Qp = act_clip_val
        else:
        ...
        # eps = torch.tensor(1e-5).float().to(alpha.device)
        # if not alpha.initialized:
        #     alpha.initialize_wrapper(input, act_clip_val, symmetric=symmetric, init_method='default')
        # alpha = alpha if alpha > eps else eps
        # alpha = torch.where(alpha > eps, alpha, eps)
        # # assert alpha > 0, 'alpha = {:.6f} becomes non-positive'.format(alpha)
        # assert (alpha > 0).all(), 'alpha has non-positive values'

        alpha = alpha.clamp(min=1e-5, max=10)
        ...
        s_q = Round.apply(torch.div(input, alpha).clamp(Qn, Qp))
        # s_q = Round.apply(input.clamp(alpha * Qn, alpha * Qp))
        out = torch.mul(s_q, alpha)
        # return torch.round(torch.clamp(input, min=0, max=act_clip_val))
        return out

    @staticmethod
    def backward(ctx, grad_output):
        input, alpha = ctx.saved_tensors
        # act_clip_val = ctx.other
        grad_scale, Qn, Qp = ctx.other

        q_s = input / alpha
        indicate_small = (q_s < Qn).float()
        indicate_big = (q_s >  Qp).float()
        indicate_middle = 1.0 - indicate_small - indicate_big  # this is more cpu-friendly than torch.ones(input_.shape)

        grad_alpha = ((indicate_small * Qn + indicate_big * Qp + indicate_middle * (
                -q_s + Round.apply(q_s))) * grad_output * grad_scale).mean(dim=[0,1], keepdim=False)   #.mean(dim=[1], keepdim=True) #.sum(dim=1).unsqueeze(dim=1) #.mean(dim=[1], keepdim=False) #.sum().unsqueeze(dim=0)  # .mean(dim=[0, 1], keepdim=False)   # .sum().unsqueeze(dim=0)
...
class enh_IFNode(BaseNode):
    def __init__(self, hids: int = 64, v_threshold: float = 1., v_reset: Optional[float] = 0.,
        ...
        self.act_quantizer = ElasticBiSpiking_MLN
        self.T = 4
        self.alpha = nn.Parameter(torch.tensor(0.43), requires_grad=True)
        self.p = nn.ParameterList([nn.Parameter(torch.tensor(1.0), requires_grad=True) for i in range(self.T)])
        # self.a = nn.Parameter(torch.ones(hids), requires_grad=True)
        # self.b = nn.Parameter(torch.zeros(hids), requires_grad=True)
        self.a = nn.Parameter(torch.full((hids,), 1.0), requires_grad=True)
        self.b = nn.Parameter(torch.full((hids,), 0.0), requires_grad=True)


    def normalize_step(self, v, eps=1e-5):
        # Calculate mean and standard deviation across all dimensions except the first one (bs)
        mean = v.mean(dim=tuple(range(1, v.dim())), keepdim=True)
        std = torch.sqrt(v.var(dim=tuple(range(1, v.dim())), unbiased=False, keepdim=True) + eps)
    ...
    @staticmethod
    @torch.jit.script
    def jit_eval_multi_step_forward_hard_reset(x_seq, v, v_threshold, v_reset, normalize_step_mn, p, act_quantizer,
                                               alpha, a, b):
        spike_seq = torch.zeros_like(x_seq)
        for t in range(x_seq.shape[0]):
            v = v + x_seq[t]
            v = normalize_step_mn(v) * (p[t].clamp(min=1e-5, max=10))
            spike = act_quantizer.apply(v, alpha).to(x_seq) * a + b
            v = v_reset * spike + (1. - spike) * v
            spike_seq[t] = spike
        return spike_seq, v
    ...
    @staticmethod
    @torch.jit.script
    def jit_eval_multi_step_forward_hard_reset_with_v_seq(x_seq, v, v_threshold, v_reset, normalize_step_mn, p, act_quantizer,
                                                          alpha, a, b):
        spike_seq = torch.zeros_like(x_seq)
        v_seq = torch.zeros_like(x_seq)
        for t in range(x_seq.shape[0]):
    ...
    @staticmethod
    @torch.jit.script
    def jit_eval_multi_step_forward_soft_reset(x_seq, v, v_threshold, normalize_step_mn, p, act_quantizer, alpha, a, b):
        spike_seq = torch.zeros_like(x_seq)
        for t in range(x_seq.shape[0]):
            v = v + x_seq[t]
            v = normalize_step_mn(v) * (p[t].clamp(min=1e-5, max=10))
    ...
    @staticmethod
    @torch.jit.script
    def jit_eval_multi_step_forward_soft_reset_with_v_seq(x_seq, v, v_threshold, normalize_step_mn, p, act_quantizer, alpha, a, b):
        spike_seq = torch.zeros_like(x_seq)
        v_seq = torch.zeros_like(x_seq)
        for t in range(x_seq.shape[0]):
            v = v + x_seq[t]
            v = normalize_step_mn(v) * (p[t].clamp(min=1e-5, max=10))
            spike = act_quantizer.apply(v, alpha).to(x_seq) * a.view(1,-1,1,1) + b
            v = v - spike * v_threshold
            spike_seq[t] = spike
            v_seq[t] = v
        return spike_seq, v, v_seq
            ...
            if self.v_reset is None:
                if self.store_v_seq:
                    spike_seq, self.v, self.v_seq = self.jit_eval_multi_step_forward_soft_reset_with_v_seq(x_seq, self.v, self.v_threshold,
                                                                self.normalize_step, self.p, self.act_quantizer, self.alpha, self.a, self.b)
                else:
                    spike_seq, self.v = self.jit_eval_multi_step_forward_soft_reset(x_seq, self.v, self.v_threshold,
                                                                self.normalize_step, self.p, self.act_quantizer, self.alpha, self.a, self.b)
            else:
                if self.store_v_seq:
                    spike_seq, self.v, self.v_seq = self.jit_eval_multi_step_forward_hard_reset_with_v_seq(x_seq,
                                                                                                           self.v,
                                                                                                           self.v_threshold,
                                                                                                           self.v_reset, self.normalize_step, self.p, self.act_quantizer, self.alpha, self.a, self.b)
                else:
                    spike_seq, self.v = self.jit_eval_multi_step_forward_hard_reset(x_seq, self.v, self.v_threshold,
                                                                                    self.v_reset, self.normalize_step, self.p, self.act_quantizer, self.alpha, self.a, self.b)
            return spike_seq
