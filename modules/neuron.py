...
class CustomRoundPass(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, scaling_factor):
        y = x.round()
        dis = x - (x.ceil() + 0.5)
        ctx.save_for_backward(dis)
        ctx.scaling_factor = scaling_factor
        return y

    @staticmethod
    def backward(ctx, grad_output):
        dis, = ctx.saved_tensors
        grad_out = grad_output.clone()
        grad_out = grad_out + ctx.scaling_factor * (0.5 - torch.sign(dis) * dis) * (grad_out / grad_out.norm(p=2))
        return grad_out, None

def round_pass(x, scaling_factor=1e-2):  # 1e-4
    return CustomRoundPass.apply(x, scaling_factor)


class RealSpiking_M(torch.nn.Module):
    def __init__(self, bits, all_positive=True, symmetric=False, per_channel=False):
        super().__init__()
        # super().__init__(bit, all_positive, symmetric, per_channel)
    ...
    def forward(self, x, alpha, p):

        if self.training and not alpha.initialized:
            alpha.initialize_wrapper(x, self.thd_neg, self.thd_pos, symmetric=self.symmetric,
                                     per_channel=self.per_channel, init_method='default')

        p_scale = grad_scale(p, 0.5)

        if self.per_channel:
            if x.dim() == 2:
                alpha_reshape = (alpha * p_scale)
            else:  # x.dim() == 4:
                alpha_reshape = (alpha * p_scale).view(1, -1, 1, 1)  # Reshape to [1, c, 1, 1]
        else:
            alpha_reshape = alpha * p_scale

        alpha_reshape = alpha_reshape.clamp(min=1e-3)
        x = x / alpha_reshape
        x = torch.clamp(x, self.thd_neg, self.thd_pos)
        round_out = round_pass(x)

        # # calulate the firing rate
        # # Fr = round_out.sum() / (torch.numel(round_out)*4)
        # Fr_v1 = (round_out[round_out >= 1].sum()) / (torch.numel(round_out) * 4)
        # Fr_v2 = (round_out[round_out >= 2].sum()) / (torch.numel(round_out) * 4)
        # Fr_v3 = (round_out[round_out >= 3].sum()) / (torch.numel(round_out) * 4)
        # Fr_v4 = (round_out[round_out >= 4].sum()) / (torch.numel(round_out) * 4)
        # Fr = (Fr_v1 + Fr_v2 + Fr_v3 + Fr_v4) / 4
        # layer_fr = [Fr, Fr_v1, Fr_v2, Fr_v3, Fr_v4]
        # layer_fr = torch.tensor(layer_fr)

        out = round_out * alpha_reshape

        return out  #, layer_fr  # layer_mem_dis



class AlphaInit_M(nn.Parameter):
    def __init__(self, tensor_in, requires_grad=False):
        # Check if tensor_in is scalar or has a specific shape
        if isinstance(tensor_in, torch.Tensor) and tensor_in.ndim != 0:
            # If tensor_in is not scalar, ensure the data matches its shape
            super(AlphaInit_M, self).__new__(nn.Parameter, data=tensor_in.clone(), requires_grad=requires_grad)
        else:
            # Default scalar initialization
            super(AlphaInit_M, self).__new__(nn.Parameter, data=torch.tensor(1.0), requires_grad=requires_grad)
    ...
    def initialize_wrapper(self, tensor_in, neg, pos, symmetric, per_channel, init_method='default'):
        Qn = neg
        Qp = pos
        ...
        if init_method == 'default':
            if per_channel:
                ...
                if tensor_in.dim() == 2:
                    # 2D tensor: Channel-wise initialization
                    mean_in = tensor_in.detach().mean(dim=[0])
                    var_in = tensor_in.detach().var(dim=[0])
                    init_val = torch.max(torch.stack(((mean_in - 3 * var_in).abs(), (mean_in + 3 * var_in).abs()), dim=0), dim=0)[0] / math.sqrt(Qp)
                elif tensor_in.dim() == 4:
                    # 4D tensor: Channel-wise initialization
                    mean_in = tensor_in.detach().mean(dim=[0, 2, 3])
                    var_in = tensor_in.detach().var(dim=[0, 2, 3])
                    init_val = torch.max(torch.stack(((mean_in - 3 * var_in).abs(), (mean_in + 3 * var_in).abs()), dim=0), dim=0)[0] / math.sqrt(Qp)
                else:
                    raise ValueError(f"Unsupported tensor dimensions for per_channel: {tensor_in.dim()}")
            else:
                # Scalar initialization
                mean_in = tensor_in.detach().mean()
                var_in = tensor_in.detach().var()
                init_val = torch.max((mean_in - 3 * var_in).abs(), (mean_in + 3 * var_in).abs()) / pos

        elif init_method == 'uniform':
            init_val_scalar = 1. / (2 * Qp + 1) if symmetric else 1. / Qp
            init_val = torch.full((tensor_in.size(1),), init_val_scalar)

        self._initialize(init_val)

    def update_wrapper(self, tensor_in, neg, pos, symmetric, per_channel, momentum=0.9):
        """
        Updates the alpha value using a moving average of the current and initial values.
        """
        if not self.initialized:
            raise RuntimeError("Alpha must be initialized before updating.")

        if per_channel:
            # Channel-wise update
            current_val = tensor_in.detach().abs().mean(dim=[0, 2, 3]) * 2 / pos
        else:
            # Scalar update
            current_val = tensor_in.detach().abs().mean() * 2 / pos

        # Moving average update
        new_alpha = momentum * self.data + (1 - momentum) * current_val
        self.data.copy_(new_alpha)

    def forward(self):
        """
        Returns the current alpha value for inference.
        """
        if not self.initialized:
            raise RuntimeError("Alpha must be initialized before forward pass.")
        return self.data



class enh_LIFNode(BaseNode):
    def __init__(self, hids: int = 64, tau: float=0.5, v_threshold: float = 1., v_reset: Optional[float] = 0.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False, step_mode='s', T: int=1, v_reset_mode: str ='soft',
                 backend='torch', store_v_seq: bool = False, **kwargs):
        """
        :param v_threshold: threshold of this neurons layer
        :type v_threshold: float
        ...
        super().__init__(v_threshold, v_reset, surrogate_function, detach_reset, step_mode, backend, store_v_seq)
        # self.step_mode = step_mode
        self._step_mode = step_mode  # Use a private attribute to store the value
        ...
        self.register_memory('time_step', 0)  # init time step counter

        self.act_quantizer = nn.ModuleList([RealSpiking_M(bits=4, all_positive=True, symmetric=False, per_channel=True) for i in range(self.T)])
        self.alpha = nn.ParameterList([AlphaInit_M(torch.ones(hids), requires_grad=False) for i in range(self.T)])
        self.p = nn.ParameterList([nn.Parameter(torch.tensor(1.0), requires_grad=True) for i in range(self.T)])

    def neuronal_charge(self, x: torch.Tensor):
        # self.v = self.tau * self.v + x    # static image
        self.v = self.tau * self.v + (1 - self.tau) * x   # event-based data

    def neuronal_fire(self, alpha: torch.Tensor, p:torch.Tensor, act_quantizer: torch.nn.Module):
        spike = act_quantizer(self.v, alpha, p)
        # spike, layer_fr = act_quantizer(self.v, alpha, p)
        return spike #, layer_fr
    ...
    def neuronal_reset(self, spike):
        ...
        # self.v = self.v - spike_d * self.v_threshold  # soft reset

        # self.v = (1. - spike_d) * self.v + spike_d * self.v_reset    # hard reset

        if self.v_reset_mode == 'soft':
            # soft reset
            self.v = self.v - spike_d * self.v_threshold
        else:
            # hard reset
            self.v = torch.where(spike_d != 0, self.v_reset, self.v)


    def single_step_forward(self, x: torch.Tensor):  # step: int
        self.v_float_to_tensor(x)
        self.neuronal_charge(x)
        spike = self.neuronal_fire(self.alpha[self.time_step], self.p[self.time_step], self.act_quantizer[self.time_step])
        # spike, layer_fr = self.neuronal_fire(self.alpha[self.time_step], self.p[self.time_step], self.act_quantizer[self.time_step])
        self.neuronal_reset(spike)
...
class LIFNode(BaseNode):
    def __init__(self, hids: int = 64, tau: float = 2., decay_input: bool = True, v_threshold: float = 1.,
                 v_reset: Optional[float] = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False, step_mode='s', backend='torch', T:int=1, store_v_seq: bool = False):
        """
            ...
            if self.v_reset is None:
                ...
                if self.decay_input:
                    ...
                    spike, self.v = self.jit_eval_single_step_forward_hard_reset_no_decay_input(x, self.v,
                                                                                                ...
                                                                                                self.v_reset,
                                                                                                self.tau)
            return spike  #, spike.sum() / (torch.numel(spike))
...
class IzhikevichNode(AdaptBaseNode):
    def __init__(self, tau: float = 2., v_c: float = 0.8, a0: float = 1., v_threshold: float = 1.,
                 v_reset: Optional[float] = 0., v_rest: float = -0.1, w_rest: float = 0., tau_w: float = 2.,
                 a: float = 0.,
                 b: float = 0.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False, step_mode='s',
                 backend='torch', store_v_seq: bool = False):
...
class GatedLIFNode(base.MemoryModule):
    def __init__(self, T: int, inplane=None,
                 init_linear_decay=None, init_v_subreset=None, init_tau: float = 0.25, init_v_threshold: float = 0.5,
        ...
        * :ref:`API in English <GatedLIFNode.__init__-en>`
        ...
        if self.channel_wise:  # channel-wise learnable params
            ...
            self.alpha, self.beta, self.gamma = [
                nn.Parameter(torch.tensor(0.2 * (np.random.rand() - 0.5), dtype=torch.float)) for i in range(3)]
    ...
    def multi_step_forward(self, x_seq: torch.Tensor):
        alpha, beta, gamma = self.alpha.view(1, -1, 1, 1).sigmoid(), self.beta.view(1, -1, 1,
                                                                                    1).sigmoid(), self.gamma.view(1, -1,
                                                                                                                  1,
                                                                                                                  1).sigmoid()
        y_seq = []
...
class OTTTLIFNode(LIFNode):
    def __init__(self, tau: float = 2., decay_input: bool = False, v_threshold: float = 1.,
        ...
        * :ref:`中文API <OTTTLIFNode.__init__-cn>`
        ...
        super().__init__(tau, decay_input, v_threshold, v_reset, surrogate_function, detach_reset, step_mode, backend,
                         store_v_seq)
...
class NoisyBaseNode(nn.Module, base.MultiStepModule):
    ...
    def reset_noise(self, num_rl_step):
        ...
        self.eps_v_seq = torch.FloatTensor(
            powerlaw_psd_gaussian(self.beta, eps_shape).reshape(self.num_node, num_rl_step, self.T)).permute(per_order)
...
class ILCBaseNode(nn.Module, base.MultiStepModule):
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        for t in range(T):
            ...
            spike_seq.append(spike)
            if t < T - 1:
                x_seq[t + 1] = x_seq[t + 1] + self.conn(spike.view(-1, self.act_dim, self.dec_pop_dim)).view(-1,
                                                                                                             self.out_pop_dim)
